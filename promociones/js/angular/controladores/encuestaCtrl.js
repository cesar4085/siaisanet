app.controller("encuestaCtrl",function($scope,$window,$timeout,anchorSmoothScroll,apiService){
     ...
     //INICIALIZACIÓN DE VARIABLES
       ...
       rome(inputCita2,{ time: true, min: new Date()}).on("data",function(value){
             $scope.interes_credito.agendar_cita=value;
             console.log("Selected date cita manejo objecion"+ $scope.interes_credito.agendar_cita);
         });
    ...
    */
      ...
      $scope.guardarLlamada=function(){
    	 ...
    	 if(typeof $scope.interes_credito.agendar_cita!="undefined"){
            var fechaCita=moment($scope.interes_credito.agendar_cita,       "YYYY-MM-DD HH:mm")
            var hora=fechaCita.format("hh:mm:ss a");
            
            if(parseInt(fechaCita.hour())>=20 || parseInt(fechaCita.hour())==0){
                $scope.isActivo=true;
                alert("Cita no valida, financiera ayudamos cierra a las 8:00 pm y tu cita es a las "+ hora);
                return;
            }
          
            else  if(fechaCita.isSame(new Date(),"day")){
                if (!confirm("¿Estás seguro? La cita que hiciste es el día de hoy a las  "+ hora)){
                       $scope.isActivo=true;
                       return;
                }
            }
                     ...
                     if (!confirm("¿Estás seguro? La cita que hiciste es el día "+ $scope.interes_credito.agendar_cita)){
                         $scope.isActivo=true;
                         return;
                     }
            }
